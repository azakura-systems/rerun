...
// Based on "crates/store/re_sdk_types/definitions/rerun/datatypes/plane3d.fbs".
...
namespace rerun {
    const std::shared_ptr<arrow::DataType>& Loggable<datatypes::Plane3D>::arrow_datatype() {
        static const auto datatype =
            arrow::fixed_size_list(arrow::field("item", arrow::float32(), false), 4);
        return datatype;
    }

    Result<std::shared_ptr<arrow::Array>> Loggable<datatypes::Plane3D>::to_arrow(
        ...
        if (instances && num_instances > 0) {
            RR_RETURN_NOT_OK(
                Loggable<datatypes::Plane3D>::fill_arrow_array_builder(
    ...
    rerun::Error Loggable<datatypes::Plane3D>::fill_arrow_array_builder(
        ...
        static_assert(sizeof(elements[0].xyzd) == sizeof(elements[0]));
        ARROW_RETURN_NOT_OK(value_builder->AppendValues(
            elements[0].xyzd.data(),
