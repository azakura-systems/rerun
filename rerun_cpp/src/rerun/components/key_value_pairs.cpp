...
namespace rerun {
    ...
    Result<std::shared_ptr<arrow::Array>> Loggable<components::KeyValuePairs>::to_arrow(
        ...
        if (instances && num_instances > 0) {
            RR_RETURN_NOT_OK(
                Loggable<components::KeyValuePairs>::fill_arrow_array_builder(
    ...
    rerun::Error Loggable<components::KeyValuePairs>::fill_arrow_array_builder(
        arrow::ListBuilder* builder, const components::KeyValuePairs* elements, size_t num_elements
    ) {
        if (builder == nullptr) {
        ...
        for (size_t elem_idx = 0; elem_idx < num_elements; elem_idx += 1) {
            ...
            if (element.pairs.data()) {
                RR_RETURN_NOT_OK(
                    Loggable<rerun::datatypes::Utf8Pair>::fill_arrow_array_builder(
                        value_builder,
                        element.pairs.data(),
                        element.pairs.size()
                    )
