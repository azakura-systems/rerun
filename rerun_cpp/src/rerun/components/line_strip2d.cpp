...
// Based on "crates/store/re_sdk_types/definitions/rerun/components/line_strip2d.fbs".
...
namespace rerun {
    ...
    Result<std::shared_ptr<arrow::Array>> Loggable<components::LineStrip2D>::to_arrow(
        ...
        if (instances && num_instances > 0) {
            RR_RETURN_NOT_OK(
                Loggable<components::LineStrip2D>::fill_arrow_array_builder(
    ...
    rerun::Error Loggable<components::LineStrip2D>::fill_arrow_array_builder(
        ...
        for (size_t elem_idx = 0; elem_idx < num_elements; elem_idx += 1) {
            ...
            if (element.points.data()) {
                RR_RETURN_NOT_OK(
                    Loggable<rerun::datatypes::Vec2D>::fill_arrow_array_builder(
                        value_builder,
                        element.points.data(),
                        element.points.size()
                    )
