...
//!
//! Simply get rid of the type in question and rebuild `re_sdk_types` to trigger codegen.
//!
...
pub mod archetypes {
    ...
    // `re_sdk_types` behemoth just so one can use one of these fundamental types.
    //
...
// One almost never uses `re_sdk_types` without `re_types_core`, so we reexport these core types
// for convenience.
// The opposite is not true, though: make sure you're not depending on `re_sdk_types` if all you need
